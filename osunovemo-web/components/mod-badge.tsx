...
export function ModBadge({ className, mod }: { className?: string; mod: DisplayMod }) {
  ...
  const title =
    hasSettings && (extendedContent === "" || settingsCount > 1)
  ...
  const content = (
    <div className={cn("relative h-6 shrink-0", extendedContent !== "" ? "w-[4.75rem]" : "w-8", className)}>
      {extendedContent !== "" ? (
        <div className="absolute top-0 left-[1.15rem] flex h-6 w-[3.6rem] items-center justify-center">
          <div
          ...
          <div
            ...
            style={{
              ...
              fontSize:
                extendedFontSize === "0.95rem"
                  ? "0.85rem"
                  : extendedFontSize === "0.85rem"
                    ? "0.75rem"
                    : extendedFontSize === "0.75rem"
                      ? "0.65rem"
                      : "0.6rem",
            }}
          >
      ...
      <div className="relative z-10 h-6 w-8 shrink-0">
        <div
          className="absolute inset-0"
          style={{
            ...baseMaskStyle,
        ...
        {hasSettings ? (
          <div className="pointer-events-none absolute left-0 -top-[5px] h-4 w-8">
            <ModCustomisedIndicator backgroundColor={colors.background} foregroundColor={colors.foreground} />
          </div>
        ) : null}
      </div>
    </div>
