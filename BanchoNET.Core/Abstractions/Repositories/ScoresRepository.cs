...
public abstract class ScoresRepository(BanchoDbContext dbContext) : IScoresRepository
{
    ...
    public async Task<List<ScoreDto>> GetPlayerBestScores(
        ...
        return await DbContext.Scores
            ...
            .Skip(offset)
            .Take(Math.Min(limit, AppSettings.TopPlaysCount))
            .ToListAsync();
    }
    ...
    public async Task<int> GetBestScoresCount(
        GameMode mode
    ) {
        // Same predicate as GetBestScores; kept next to it so the two cannot drift and report a
        // page count that does not match the pages actually served.
        return await DbContext.Scores
            .AsNoTracking()
            .CountAsync(s => s.Mode == mode
                             && (s.Player.Privileges & 1) == 1
                             && (s.Beatmap.Status == BeatmapStatus.Ranked || s.Beatmap.Status == BeatmapStatus.Approved)
                             && s.Status == SubmissionStatus.Best
                             && s.Ranked);
