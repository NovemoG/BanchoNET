...
namespace BanchoNET.Handlers.Stable.Controllers.Cho;
...
public partial class ChoController
{
	private async Task<IActionResult> Login()
	{
		...
		var geoloc = await geolocService.GetGeoloc(Request.Headers);
		if (geoloc == null)
		{
		...
		var ip = geolocService.GetIp(Request.Headers);
		...
		if (userInfo.Country == "xx")
			await players.UpdatePlayerCountry(player, geoloc.Value.Country.Acronym);
